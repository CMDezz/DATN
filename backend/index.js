...
Product.find()
  .then(prx => {
    prx.map(pr => {
      // pr.productFinalPrice = pr.productPrice - (pr.productPrice * pr.productSale / 100);
      pr.productGroupCate = "";
      pr.save();
    });
...
io.on("connection", async function(socket) {
  ...
  socket.on("messageSend", async function(data) {
    Chat.findOne({ sessionId: data.sessionId })
      .updateOne({
        $push: { chatContent: { text: data.text, time: data.time } }
      })
      .exec();
  ...
  socket.on("messageSend-admin", function(data) {
    Chat.findOne({ sessionId: data.roomId })
      .updateOne({
        $push: {
          chatContent: { fromAdmin: true, text: data.text, time: data.time }
        }
      })
      .exec();
